...
st.set_page_config(
    page_title='Data_Analytics Portal',
    page_icon= '📊'
)
st.title(':rainbow[Data Analysis Portal]')
st.subheader(':gray[Exploare Data with ease.]',divider='rainbow')
file = st.file_uploader('Drop CSV or Excel file]',type=['csv','xlsx','xlsb'])
if(file!=None):
    ...
    st.dataframe(data)
    st.info('file is sucessfully uploaded',icon='🔦')
    st.subheader(':rainbow[Basic information of the dataset]',divider='rainbow')
    tab1,tab2,tab3,tab4 = st.tabs(['Summary','Top & Bottom Rows','Data types','Coloums'])
    ...
    with tab2:
        tab1,tab2=st.tabs(["Toprows ⬆️","Bottomrows ⬇️"])
        with tab1:
    ...
    with tab4:
        st.subheader(':gray[Columns name in Dataset]')
        st.write(list(data.columns))
    st.subheader(':rainbow[Column Values To Count]',divider='rainbow')
    with st.expander('Value Count'):
        col1,col2=st.columns(2)
        with col1:
            column =st.selectbox('Chooes Column name', options=list(data.columns))
        with col2:
        ...
        if(count==True):
            ...
            fig =px.bar(data_frame=result,x=column,y="count",text="count",template="gridon")  #presentation
            st.plotly_chart(fig)
    ...
    with st.expander('Group By your columns'):
        ...
        with col1:
            groupby_cols =st.multiselect("Chooes your column to groupby",options=list(data.columns))
        with col2:
